...
async function transmitHandler(orgId: string, userId: string, id: string) {
...
async function ingestHandler(orgId: string, userId: string, id: string, body: unknown) {
...
const actionSchema = z.object({
  action: z.enum(["transmit", "ingest"]),
  payload: z.unknown().optional(),
});

async function guard(orgId: string) {
  const moduleAuthz = await requireModulePermission(orgId, "labs");
  if (moduleAuthz.response) return { error: moduleAuthz.response as NextResponse };
  ...
  if (authz.response) return { error: authz.response as NextResponse };
  return { userId: authz.userId as string };
}
...
export async function POST(
  ...
  try {
    ...
    const parsed = actionSchema.safeParse({ action: verb, payload: body });
    if (!parsed.success) {
      return NextResponse.json(
        { error: "Validation failed", details: parsed.error.flatten() },
        { status: 400 },
    ...
    if (verb === "transmit") return transmitHandler(orgId, g.userId, id);
    if (verb === "ingest") return ingestHandler(orgId, g.userId, id, body);

    return NextResponse.json({ error: "Unknown action" }, { status: 404 });
  } catch (error) {
    logServerError("Lab exchange failed", error);
    return NextResponse.json({ error: "Lab exchange failed" }, { status: 500 });
  }
}
