  ...
  return (
    <section className="mt-6 max-w-3xl space-y-4">
      ...
      <div className="space-y-5 rounded-lg border border-border bg-card p-6">
        ...
        <label className="block"><span className="mb-1 block text-sm font-semibold text-foreground">{t["super_defaultPlan"]}</span><SearchableSelect value={settings.defaultPlan} onValueChange={(v) => setSettings((current) => ({ ...current, defaultPlan: v }))} options={plans.map((plan) => ({ value: plan.code, label: plan.nameEn ?? plan.code }))} placeholder={t["super_defaultPlan"]} triggerClassName="w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground" /></label>
