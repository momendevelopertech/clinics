...
export const ROLES_GUIDE: GuideRole[] = [
  {
    ...
    tasks: [
      ...
      },
      {
        id: "owner-catalogs",
        icon: "ClipboardList",
        title: t("كتالوج الخدمات والأكواد السريرية (إدارة كاملة)", "Service and clinical catalogs (full management)"),
        where: t("صفحة /catalogs — قوائم الخدمات والسريرية مع بحث وأزرار الإضافة/التعديل/الحذف (للمالك فقط)", "Page /catalogs — service/clinical lists with search and Add/Edit/Delete buttons (Owner only)"),
        steps: [
          t("يفتح /catalogs ويرى الخدمات (code·name + price) والأكواد السريرية مع بحث وصفحات.", "Opens /catalogs to browse services (code·name + price) and clinical codes with search and pagination."),
          t("للإضافة: يملأ code وname وprice (+ category) ويدوس Add فينفذ POST /api/catalogs?kind=service (والسريري POST ?kind=clinical بالـ {system, code, name, category}).", "To add: fills code, name, price (+ category), clicks Add, firing POST /api/catalogs?kind=service (clinical: POST ?kind=clinical with {system, code, name, category})."),
          t("للتعديل: قلم التعديل يملأ الفورم ثم Save فينفذ PATCH /api/catalogs/[id]?kind=.", "To edit: the pencil pre-fills the form, then Save fires PATCH /api/catalogs/[id]?kind=."),
          t("للحذف: common_delete بتأكيد فينفذ DELETE — الخدمة المرتبطة بسجلات تُعطَّل (active=false) بدل حذفها.", "To delete: common_delete with confirm fires DELETE — a service referenced by records is deactivated (active=false) instead of deleted."),
          t("غير المالك يرى نفس الصفحة قراءة فقط مع ملاحظة Only the Owner can manage — أزرار الكتابة مخفية.", "Non-Owners see the same page read-only with an Only the Owner can manage note — write buttons hidden."),
        ],
        backend: t("القراءة بموديول catalogs، الكتابة requireOwner (وإلا 403)، والتحقق Zod، وتعطيل المرتبط بدل حذفه، وتدقيق CREATE/UPDATE catalog.", "Reads need catalogs module, writes requireOwner (else 403), Zod-validated; referenced rows deactivated instead of deleted; audit CREATE/UPDATE catalog."),
        result: t("القوائم تتحدث فورًا، والأسعار والأكواد الجديدة تُستخدم في الفواتير والأوامر والتحاليل.", "Lists refresh at once; new prices/codes flow into invoices, orders and labs."),
      },
      {
        id: "owner-providers",
        icon: "ShieldCheck",
        title: t("مزودو التأمين (إضافة وتعديل وتعطيل)", "Insurance providers (add, edit, deactivate)"),
        where: t("صفحة /insurance — بطاقة Providers العلوية: حقل الاسم وزرار الحفظ والتعديل والتعطيل", "Page /insurance — top Providers card: name field, save/edit/deactivate actions"),
        steps: [
          t("يفتح /insurance فيرى بطاقة Providers (5 مزودين مبذورين: Allianz وAXA وMedNet وNextCare وGlobeMed Egypt).", "Opens /insurance to see the Providers card (5 seeded providers: Allianz, AXA, MedNet, NextCare, GlobeMed Egypt)."),
          t("للإضافة: يكتب الاسم ويدوس Save فينفذ POST /api/insurance/providers بالـ {name}.", "To add: types the name, clicks Save, firing POST /api/insurance/providers with {name}."),
          t("لإعادة التسمية: التعديل ثم Save فينفذ PATCH …/[id] — الاسم الجديد يسري على كل السياسات القديمة (cascade).", "To rename: edits then Save fires PATCH …/[id] — the new name cascades onto all existing policies."),
          t("للتعطيل: common_delete بتأكيد فينفذ DELETE فيقلب active=false (لا حذف فيزيائي).", "To deactivate: common_delete with confirm fires DELETE, flipping active=false (no physical delete)."),
          t("غير المالك يرى المزودين في قائمة منسدلة فقط عند إنشاء سياسة — بلا بطاقة إدارة.", "Non-Owners only see providers as a dropdown when creating a policy — no management card."),
        ],
        backend: t("الكتابة requireOwner (وإلا 403)، وتفرد الاسم لكل مؤسسة (تكرار 409)، وإعادة التسمية تحدّث السياسات في نفس المعاملة، وتدقيق CREATE/UPDATE.", "Writes requireOwner (else 403), per-org unique names (dup → 409); rename updates policies in the same transaction; audit CREATE/UPDATE."),
        result: t("قائمة المزودين والقائمة المنسدلة في نموذج السياسة تتحدثان فورًا.", "Providers list and the policy-form dropdown refresh at once."),
      },
      {
        id: "owner-override",
        icon: "Crown",
        title: t("تجاوز المالك: تعديل/حذف أي سجل في مؤسسته", "Owner Override: edit/delete any record in their org"),
        where: t("أي صفحة سجلات — نفس أزرار التعديل والحذف العادية (بلا واجهة خاصة)", "Any records page — the same ordinary edit/delete buttons (no special UI)"),
        steps: [
          t("يفتح أي سجل في مؤسسته (حتى لو أنشأه دكتور أو ممرضة أو محاسب) ويدوس تعديل أو حذف.", "Opens any record in their organization (even one created by a Doctor, Nurse or Biller) and clicks edit or delete."),
          t("الطلب يُقبل لأن المالك يتجاوز شرط المُنشئ — نفس الـ API ونفس الأزرار بلا خطوة إضافية.", "The request succeeds because the Owner bypasses the creator check — same API, same buttons, no extra step."),
          t("سجل مؤسسة أخرى مرفوض تمامًا (404/403) — العزل بين المؤسسات مطلق.", "Another organization's record is strictly rejected (404/403) — cross-org isolation is absolute."),
        ],
        backend: t("فحص requireOwner يتجاوز ملكية المُنشئ داخل نفس organizationId فقط، مع نطاق org إلزامي في كل استعلام، وتدقيق UPDATE/DELETE. مثبت بخمسة اختبارات في tests/integration/owner-override.test.ts.", "requireOwner check bypasses creator ownership inside the same organizationId only, with mandatory org scoping on every query, audited UPDATE/DELETE. Proven by five tests in tests/integration/owner-override.test.ts."),
        result: t("السجل يتحدث أو يُحذف فورًا داخل مؤسسته فقط.", "Record updates or deletes at once — inside their org only."),
      },
    ],
    boundaries: [
      ...
      t("تجاوز المالك داخل مؤسسته فقط — سجلات المؤسسات الأخرى محظورة تمامًا (عزل كامل).", "Owner Override works inside their org only — other orgs' records are strictly forbidden (full isolation)."),
      t("لا جداول رواتب — غير موجودة.", "No payroll tables — they do not exist."),
    ],
  },
  {
    id: "super-admin",
  ...
  },
  {
    ...
    tasks: [
      ...
      },
      {
        ...
        steps: [
          ...
          t("3 قوالب SOAP مبذورة جاهزة (HTN وPediatric URI وAntenatal).", "3 SOAP templates pre-seeded (HTN, Pediatric URI, Antenatal)."),
      ...
      },
      {
        id: "doctor-rx-tools",
        icon: "Send",
        title: t("إرسال الروشتة والتكرار والنسخ والمفضلة والقوالب الخاصة", "Send, repeat, clone, favorites, private templates"),
        ...
        steps: [
          ...
          t("القوالب خاصة افتراضيًا: GET /api/prescription-templates يعرض المشتركة + قوالبه فقط (المالك يرى الكل).", "Templates are private by default: GET /api/prescription-templates shows shared plus own templates only (Owner sees all)."),
          t("4 قوالب مبذورة جاهزة (HTN وDiabetes مشتركتان، URI وPediatric fever خاصتان). قالب زميله يظهر بشارة shared، وعند الحفظ يختار مفتاح المشاركة (templateShared).", "4 templates pre-seeded (HTN and Diabetes shared; URI and Pediatric fever private). A colleague's template shows a shared badge; on save the share toggle (templateShared) shares it."),
          t("جنب اسم الدواء أيقونة ↗ تفتح /inventory في تبويب جديد (لمن يملك المخزون) وإلا تلميح Managed by Nurse / Pharmacist.", "Next to the medication name a ↗ icon opens /inventory in a new tab (for stock holders), else a Managed by Nurse / Pharmacist tooltip."),
        ],
        backend: t("الإرسال يبني رسالة ويبعث SMS/WhatsApp ويسجل Communication + تدقيق (400 بلا هاتف). القوالب: isShared=false للمؤلف فقط، والمالك يتجاوز.", "Send builds the message, delivers SMS/WhatsApp, records Communication + audit (400 without phone). Templates: isShared=false visible to author only; Owner bypasses."),
      ...
      },
      {
        ...
        steps: [
          t("يختار اسم التحليل من قائمة الكتالوج السريري system=LAB (9 مبذورة: CBC وFBS وHbA1c وLipid وTSH وUrine وVITD وCXR وABDUS) أو يكتب نصًا مخصصًا عند غيابه.", "Picks the test name from the clinical catalog picker system=LAB (9 seeded: CBC, FBS, HbA1c, Lipid, TSH, Urine, VITD, CXR, ABDUS), or types custom text as fallback."),
          t("جنب حقل التحليل أيقونة ↗ تفتح /catalogs في تبويب جديد (كتالوج المالك).", "Next to the test field a ↗ icon opens /catalogs in a new tab (Owner's catalog)."),
      ...
      },
      {
        ...
        where: t("صفحتا /tasks (CreateTaskDialog مع قائمة Type) و/audit (بحث وفلترة)", "Pages /tasks (CreateTaskDialog with Type dropdown) and /audit (search + filters)"),
        steps: [
          t("ينشئ مهمة عبر POST /api/tasks بالـ {title, priority, taskType?, dueDate?, patientId?} ويكملها PATCH {status:completed}.", "Creates a task via POST /api/tasks with {title, priority, taskType?, dueDate?, patientId?}; completes via PATCH {status:completed}."),
          t("يختار النوع من القائمة: follow_up أو lab_review أو claim_followup — نوع follow_up يشغّل تصعيد المتابعة التلقائي (cron).", "Picks the Type dropdown: follow_up, lab_review or claim_followup — follow_up feeds the automatic follow-up escalation cron."),
    ...
    boundaries: [
      ...
      t("الكتالوجات قراءة فقط — الإضافة والتعديل والحذف للمالك في /catalogs.", "Catalogs are read-only — add/edit/delete is Owner-only in /catalogs."),
    ...
    tasks: [
      ...
      },
      {
        ...
        steps: [
          ...
          t("يرسل ويدير المفضلة والقوالب (نفس أدوات الدكتور — القوالب خاصة افتراضيًا مع شارة shared ومفتاح templateShared).", "Sends and manages favorites/templates (same Doctor tools — templates private by default with shared badge and templateShared toggle)."),
          t("أيقونة ↗ جنب اسم الدواء تفتح /inventory في تبويب جديد مباشرة (يملك المخزون).", "The ↗ icon next to the medication name opens /inventory in a new tab directly (holds inventory)."),
        ],
      ...
      },
      {
        ...
        where: t("صفحة /labs — نفس ديالوجات وأزرار الدكتور (اسم التحليل قائمة كتالوج + ↗)", "Page /labs — same dialogs and buttons as Doctor (test name is a catalog picker + ↗)"),
        steps: [
          t("يختار اسم التحليل من قائمة الكتالوج السريري system=LAB (9 مبذورة) أو يكتب نصًا مخصصًا، مع أيقونة ↗ لـ /catalogs.", "Picks the test name from the clinical catalog picker system=LAB (9 seeded) or types custom text, with a ↗ icon to /catalogs."),
      ...
      },
      {
        ...
        steps: [
          ...
          t("المهام: إنشاء وإكمال مع اختيار النوع (follow_up/lab_review/claim_followup) من قائمة Type.", "Tasks: create and complete with Type pick (follow_up/lab_review/claim_followup) from the dropdown."),
        ],
      ...
      },
      {
        ...
        steps: [
          ...
          t("يرى الكتالوجات فقط — الإضافة والتعديل والحذف للمالك (ملاحظة Only the Owner can manage).", "Views catalogs only — add/edit/delete is Owner-only (Only the Owner can manage note)."),
        ],
  ...
  },
  {
    ...
    tasks: [
      ...
      },
      {
        ...
        where: t("صفحة /tasks (CreateTaskDialog مع قائمة Type) وتبويب day في /appointments", "Page /tasks (CreateTaskDialog with Type dropdown) and day tab in /appointments"),
        steps: [
          t("ينشئ مهمة عبر POST /api/tasks بالـ {title, priority?, taskType?, dueDate?, patientId?} مع اختيار النوع (follow_up/lab_review/claim_followup).", "Creates a task via POST /api/tasks with {title, priority?, taskType?, dueDate?, patientId?}, picking the Type (follow_up/lab_review/claim_followup)."),
  ...
  },
  {
    ...
    tasks: [
      {
        id: "biller-invoice",
        icon: "FileText",
        title: t("إنشاء فاتورة من خدمة الكتالوج", "Create an invoice from a catalog service"),
        where: t("صفحة /billing — زرار New Invoice (Plus) وديالوج الإنشاء مع قائمة الخدمات وأيقونة ↗", "Page /billing — New Invoice button (Plus) and create dialog with services picker and ↗ icon"),
        steps: [
          t("يختار المريض ثم خدمة من قائمة الكتالوج (code·name + price) — السعر والوصف من الكتالوج وserviceCatalogId يُرسل مع البند.", "Selects the patient, then a service from the catalog picker (code·name + price) — price and description come from the catalog; serviceCatalogId is sent with the line."),
          t("أيقونة ↗ جنب حقل الخدمة تفتح /catalogs في تبويب جديد (كتالوج المالك) — أو تلميح لغير المخول.", "The ↗ icon next to the service field opens /catalogs in a new tab (Owner's catalog) — or a tooltip for unauthorized roles."),
          t("الوصف اليدوي احتياطي فقط عند فراغ الكتالوج أو تعذّر تحميله (ds_noCatalogServices).", "Manual description is only a fallback when the catalog is empty or unreachable (ds_noCatalogServices)."),
          t("يدوس Create فينفذ POST /api/billing/invoices بالـ {patientId, lineItems:[{serviceCatalogId?, description, quantity, unitPrice?}], couponCode?, idempotencyKey}.", "Clicks Create, firing POST /api/billing/invoices with {patientId, lineItems, couponCode?, idempotencyKey}."),
        ],
      ...
      },
      {
        ...
        where: t("صفحة /insurance — بطاقة Providers + تبويبا السياسات/المطالبات ونموذج إنشاء بقائمة مزودين منسدلة", "Page /insurance — Providers card + policies/claims tabs and create form with provider dropdown"),
        steps: [
          t("يختار المزود من قائمة منسدلة (GET /api/insurance/providers — النشطون فقط، 5 مبذورين) بدل كتابة الاسم يدويًا.", "Picks the provider from a dropdown (GET /api/insurance/providers — active only, 5 seeded) instead of typing the name."),
          t("جنب حقل المزود أيقونة ↗ تفتح /insurance (بطاقة Providers التي يديرها المالك).", "Next to the provider field a ↗ icon opens /insurance (the Owner-managed Providers card)."),
      ...
      },
      {
        id: "biller-coupons",
        icon: "Copy",
        title: t("الكوبونات: بطاقة إنشاء وتفعيل + استخدام في الفاتورة", "Coupons: create/activate card + apply in invoice"),
        where: t("صفحة /billing — بطاقة CouponsCard (إنشاء percent/fixed وتفعيل بالنقر) + حقل Coupon code في ديالوج الفاتورة", "Page /billing — CouponsCard (create percent/fixed, click-chip activate) + Coupon code field in invoice dialog"),
        steps: [
          t("ينشئ كوبونًا من البطاقة: code وkind (percent بحد 100 أو fixed) وvalue (+ expiresAt) فينفذ POST /api/coupons.", "Creates a coupon from the card: code, kind (percent capped at 100 or fixed), value (+ expiresAt), firing POST /api/coupons."),
          t("يفعّل/يعطّل بالنقر على شارة الكوبون (PATCH /api/coupons/[id]) — 3 كوبونات مبذورة جاهزة.", "Activates/deactivates by clicking the coupon chip (PATCH /api/coupons/[id]) — 3 coupons pre-seeded."),
          t("يكتب الكود في حقل الفاتورة فيُفحص (نشط وغير منتهٍ) ويُخصم عبر applyCouponDiscount.", "Types the code in the invoice field; validated (active, unexpired) and discounted via applyCouponDiscount."),
        ],
        backend: t("الحماية billing. percent ≤100، وتكرار الكود 409، وتدقيق CREATE/UPDATE Coupon.", "billing guard. percent ≤100, duplicate code 409, audit CREATE/UPDATE Coupon."),
        result: t("البطاقة تتحدث، والكود يُحفظ على الفاتورة (couponCode + orderDiscount).", "Card refreshes; code stored on invoice (couponCode + orderDiscount)."),
      },
      {
        id: "biller-reports",
        icon: "Activity",
      ...
      },
      {
        ...
        where: t("صفحة /billing — بحث billing_search وفلتر الحالة + صفحة /tasks (قائمة Type)", "Page /billing — billing_search + status filter; page /tasks (Type dropdown)"),
        steps: [
          ...
          t("يدير المهام عبر /tasks (نفس كل الأدوار — مع اختيار taskType من القائمة).", "Manages tasks via /tasks (same as all roles — with taskType pick from the dropdown)."),
        ],
    ...
    boundaries: [
      t("لا يرى الزيارات والتحاليل والروشتات والمخزون (الدكتور/الممرضة/الصيدلي) — ولا يفتح /inventory إطلاقًا.", "No encounters/labs/prescriptions/inventory (Doctor/Nurse/Pharmacist) — never opens /inventory."),
      ...
      t("إلغاء التقسيط وجدولة التقارير: API/مالك فقط.", "Installment cancel and report scheduling: API/Owner-only."),
    ...
    tasks: [
      {
        id: "pharm-rx",
        icon: "Pill",
        title: t("مراجعة الروشتات وصرفها", "Review and dispense prescriptions"),
        where: t("صفحة /prescriptions — الجدول والأزرار (ظاهرة دائمًا) + ديالوج الصرف DispenseDialog", "Page /prescriptions — table and buttons (always rendered) + DispenseDialog"),
        steps: [
          ...
          t("للصرف: يختار المخزون لكل سطر في DispenseDialog (POST /api/prescriptions/[id]/dispense) — جنب الاختيار أيقونة ↗ تفتح /inventory.", "To dispense: picks stock per line in DispenseDialog (POST /api/prescriptions/[id]/dispense) — a ↗ icon next to the picker opens /inventory."),
          t("القوالب خاصة افتراضيًا (شارة shared لقوالب الزملاء) — يرى المشتركة + قوالبه فقط.", "Templates are private by default (shared badge on colleagues' templates) — sees shared plus own only."),
        ],
      ...
      },
      {
        ...
        steps: [
          ...
          t("يضيف نتيجة/طلبًا بنفس حقول الدكتور — اسم التحليل قائمة كتالوج system=LAB (9 مبذورة) أو نص مخصص، مع ↗ لـ /catalogs.", "Adds results/orders with Doctor's fields — test name is a system=LAB catalog picker (9 seeded) or custom text, with ↗ to /catalogs."),
      ...
      },
      {
        ...
        steps: [
          ...
          t("المهام: إنشاء وإكمال بنفس كل الأدوار مع اختيار taskType (مع المنح).", "Tasks: create/complete like all roles with taskType pick (with grants)."),
        ],
